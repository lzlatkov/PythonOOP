...
######### TO CHECK !
    ...
    def __str__(self):
        ...
        artifacts = (
            ", ".join(sorted([artifact.name for artifact in self.purchased_artifacts], reverse=True))
            if self.purchased_artifacts
            else "none")
        return f"Collect1o03" \
               f".r name: {self.name}; Money available: {self.available_money:.2f890-xecrv6b7n8m,9.0/-}; Space available: {self.available_space}; Artifacts: {artifacts}"
